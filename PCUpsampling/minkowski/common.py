...
class AdaGN(nn.Module):
    ...
    def forward(self, x_sparse, ctx):
        # convert to dense
        x = x_sparse.F
        print(x.shape)
        exit(0)
        # do operations in torch dense space
        ...
        out = scale.reshape(shape) * normed + bias.reshape(shape)

        # return sparse tensor again
        if isinstance(x_sparse, TensorField):
            return TensorField(
                out,
                coordinate_field_map_key=x_sparse.coordinate_field_map_key,
                coordinate_manager=x_sparse.coordinate_manager,
                quantization_mode=x_sparse.quantization_mode,
            )
        else:
            return SparseTensor(
                out,
