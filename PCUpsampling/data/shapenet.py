...
from .utils import normalize_lowres_hires_pair


def get_dataset_shapenet(dataroot, npoints, category, upsample_frac=2, overfit=False):
    tr_dataset = ShapeNet15kPointCloudsUpsampling(
        root_dir=dataroot,
        categories=[category],
        split="train",
    ...
    te_dataset = ShapeNet15kPointCloudsUpsampling(
        root_dir=dataroot,
        categories=[category],
        split="val",
        tr_sample_size=npoints,
...
class ShapeNet15kPointCloudsUpsampling(Uniform15KPC):
    def __init__(
        self,
        ...
        self.upsample_frac = upsample_frac

        super(ShapeNet15kPointCloudsUpsampling, self).__init__(
            root_dir,
            self.synset_ids,
    ...
    def __getitem__(self, idx):
        ...
        te_out = te_out[te_idxs, :]

        # subsample to get the lowres pointcloud and add a little bit or noise
        tr_out_lowres_idx = np.random.choice(tr_out.shape[0], self.tr_sample_size // self.upsample_frac)
        tr_out_lowres = tr_out[tr_out_lowres_idx]

        noise = np.random.randn(tr_out_lowres.shape[0], tr_out_lowres.shape[1]) * 0.005
        tr_out_lowres = tr_out_lowres + noise

        # repeat the points to get the same number of points as the highres pointcloud
        tr_out_lowres = np.repeat(tr_out_lowres, self.upsample_frac, axis=0)

        # normalize the lowres and highres pointclouds
        tr_out, tr_out_lowres = normalize_lowres_hires_pair(tr_out, tr_out_lowres)

        out = {
            "idx": idx,
            "train_points": torch.from_numpy(tr_out).float(),
            "train_points_lowres": torch.from_numpy(tr_out_lowres).float(),
        }
