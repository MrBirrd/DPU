...
class I2SB(DiffusionModel):
    def __init__(self, cfg):
        ...
        # load model
        self.net = load_model(cfg)
        # setup betas
        betas = make_beta_schedule(n_timestep=cfg.diffusion.timesteps, linear_end=cfg.diffusion.beta_max / cfg.diffusion.timesteps)
    ...
    def sample_ddpm(self, steps, pred_x0_fn, x1, ot_ode=False, log_steps=None, verbose=True):
    ...
    @torch.no_grad()
    def ddpm_sampling(self, x1, cond=None, clip_denoise=False, nfe=None, log_count=10, verbose=True):
        ...
        xs, pred_x0 = self.sample_ddpm(
            steps, pred_x0_fn, x1, ot_ode=self.ot_ode, log_steps=log_steps, verbose=verbose,
        )
    ...
    def sample(self, shape, cond=None, x_start=None, return_noised_hint=False, clip=False):
        if self.cfg.diffusion.sampling_type == "DDPM":
            chain, pred =  self.ddpm_sampling(
                x1=x_start,
                cond=cond,
                clip_denoise=clip,
                nfe=self.cfg.diffusion.sampling_steps,
                verbose=False
            )
    ...
    def forward(self, x0, x1, cond=None, *args, **kwargs):
        step = torch.randint(0, self.timesteps, (x0.shape[0],))
        xt = self.q_sample(step, x0, x1, ot_ode=self.ot_ode)
        gt = self.compute_gt(step, x0, xt)

        pred = self.net(xt, step, cond=cond)
        assert xt.shape == gt.shape == pred.shape
