...
class ElucidatedDiffusion(nn.Module):
    def __init__(
        ...
        if args.model.type == "PVD":
            ...
            set_transformer = SetTransformer(n_layers=args.model.ST.layers, feature_dim=args.model.ST.fdim, num_inducers=args.model.SR.inducers, t_embed_dim=1).cuda()
            self.net = LinearLift(inner=set_transformer, feature_dim=args.model.ST.fdim, in_dim=args.model.in_dim + args.model.extra_feature_channels, out_dim=args.model.out_dim,).cuda()
        else:
        ...
        self.num_sample_steps = (
            args.diffusion.sampling_timesteps
        )  # otherwise known as N in the paper
    ...
    def preconditioned_network_forward(
        self, noised_images, sigma, cond=None, clamp=False
    ):
        ...
        **kwargs
        ...
        for sigma, sigma_next, gamma in tqdm(
            ...
            model_output = self.preconditioned_network_forward(
                images_hat, sigma_hat, cond=cond, clamp=clip_denoised
            )
    ...
    def sample_using_dpmpp(self, batch_size=16, cond=None, num_sample_steps=None):
        ...
        for i in tqdm(range(len(sigmas) - 1)):
            denoised = self.preconditioned_network_forward(
                images, sigmas[i].item(), cond=cond
            )
