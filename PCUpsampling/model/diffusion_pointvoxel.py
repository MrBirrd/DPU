...
class PVD(nn.Module):
    def __init__(
        ...
        betas = get_betas(
            cfg.diffusion.schedule, cfg.diffusion.beta_start, cfg.diffusion.beta_end, cfg.diffusion.timesteps
        )
        self.diffusion = GaussianDiffusion(
            betas, loss_type, model_mean_type, model_var_type, device=device, amp=cfg.training.amp
        )
...
def get_betas(schedule_type, b_start, b_end, time_num):
    if schedule_type == "linear":
        ...
        betas = sigmoid_beta_schedule(time_num, tau=1.0).numpy()
...
class GaussianDiffusion:
    ...
    def ddim_sample(
        self, shape, denoise_fn, cond=None, save_every=0, sampling_timesteps=50, clip=False, *args, **kwargs
    ):
        batch, total_timesteps, eta = shape[0], self.num_timesteps, 0.0
        ...
        for time, time_next in tqdm(time_pairs, desc="DDIM sampling loop time step"):
            ...
            pred_noise, x_start = self.model_prediction(
                denoise_fn=denoise_fn, data=img, t=time_cond, cond=cond, clip_denoised=clip
            )
    ...
    def p_sample_loop(
        ...
        for t in tqdm(
            reversed(range(0, self.num_timesteps if not keep_running else len(self.betas))),
            desc="DDPM sampling",
