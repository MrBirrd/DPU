...
class RINBlock(nn.Module):
    def __init__(
        ...
        **attn_kwargs,
        ...
        self.latents_attend_to_pc = Attention(
            dim_latent, dim_context=pc_dim, norm=True, norm_context=True, **attn_kwargs
        )
        ...
        self.pc_attend_to_latents = Attention(
            pc_dim, dim_context=dim_latent, norm=True, norm_context=True, **attn_kwargs
        )
...
if __name__ == "__main__":
    ...
    times = torch.zeros((B,), device="cuda").float().uniform_(0, 1.0)
