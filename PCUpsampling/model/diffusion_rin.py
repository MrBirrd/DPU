...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        scale=1.0,  # this will be set to < 1. for better convergence when training on higher resolution images
    ):
        super().__init__()
    ...
    def ddpm_sample(self, shape, cond=None, time_difference=None):
        ...
        for time, time_next in tqdm(time_pairs, desc="sampling loop time step", total=self.timesteps):
            ...
            model_output, last_latents = self.model(
                maybe_normalized_img,
                noise_cond,
                cond=cond,
                x_self_cond=x_start,
