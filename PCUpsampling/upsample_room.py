...
from utils.args import parse_args, args_to_string
...
def main():
    ...
    for gpu_batch in tqdm(range(0, n_gpu_batches)):
        ...
        for data in batch_data:
            ...
            batch_points = torch.cat([batch_points, torch.from_numpy(data["iphone"][:,:3].T).unsqueeze(0).cuda()], dim=0)
            batch_gt = torch.cat([batch_gt, torch.from_numpy(data["faro"][:,:3].T).unsqueeze(0).cuda()], dim=0)
            batch_features = torch.cat([batch_features, torch.from_numpy(data[features_key].T).unsqueeze(0).cuda()], dim=0)
