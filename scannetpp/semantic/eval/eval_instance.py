...
def assign_instances_for_scan(pred_file, gt_file, preds_dir, ignore_mask, label_info, eval_opts):
    ...
    gt_instances = instance_utils.get_instances(
        gt_ids, label_info.valid_class_ids, label_info.class_labels, label_info.id_to_label
    )
...
def evaluate(pred_files, gt_files, preds_dir, data_root, label_info, eval_opts):
    ...
    for scene_ndx, pred_file in enumerate(tqdm(pred_files, desc="assign_pred_scene")):
        ...
        gt2pred, pred2gt = assign_instances_for_scan(
            pred_file, gt_files[scene_ndx], preds_dir, ignore_mask, label_info, eval_opts
        )
...
def verify_pred_files(pred_files, label_info):
    ...
    for pred_file in tqdm(pred_files, desc="check_pred_files"):
        ...
        for line_ndx, line in enumerate(tqdm(lines, desc="check_mask_file", leave=False)):
            ...
            assert int(line[1]) in range(
                len(label_info.all_class_ids)
...
def eval_instance(scene_ids, preds_dir, gt_dir, data_root, label_info, eval_opts, check_pred_files=False):
...
def main(args):
    ...
    results = eval_instance(
        scene_ids, cfg.preds_dir, cfg.gt_dir, cfg.data_root, label_info, eval_opts, cfg.check_pred_files
    )
    # print everything
