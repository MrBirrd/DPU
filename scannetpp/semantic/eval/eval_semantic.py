...
def eval_semantic(
    scene_list, pred_dir, gt_dir, data_root, num_classes, ignore_label, top_k_pred=[1, 3], eval_against_gt=False
):
    ...
    confmats = {k: ConfMat(num_classes, top_k_pred=k, ignore_label=ignore_label) for k in top_k_pred}
...
def main(args):
    ...
    confmats = eval_semantic(
        scene_ids, cfg.preds_dir, cfg.gt_dir, cfg.data_root, num_classes, -100, [1, 3], eval_against_gt=eval_against_gt
    )
